...
ENV NOMAD_VERSION 0.9.7%2bent

RUN set -x \
  && apk --update add --no-cache --virtual .nomad-deps gnupg curl \
  && cd /tmp \
  && curl -L -o nomad_${NOMAD_VERSION}_linux_amd64.zip https://cdn.aws.robloxlabs.com/nomad/${NOMAD_VERSION}/nomad-enterprise_${NOMAD_VERSION}_linux_amd64.zip\
  && curl -L -o nomad_${NOMAD_VERSION}_SHA256SUMS      https://cdn.aws.robloxlabs.com/nomad/${NOMAD_VERSION}/nomad-enterprise_${NOMAD_VERSION}_linux_amd64.zip \
  && curl -L -o nomad_${NOMAD_VERSION}_SHA256SUMS.sig  https://cdn.aws.robloxlabs.com/nomad/${NOMAD_VERSION}/nomad-enterprise_${NOMAD_VERSION}_linux_amd64.zip \
  && export GNUPGHOME="$(mktemp -d)" \
