...
RUN set -x \
  ...
  && cd /tmp \
  && curl -L -o nomad_${NOMAD_VERSION%2bent}_linux_amd64.zip https://cdn.aws.robloxlabs.com/nomad/${NOMAD_VERSION%2bent}/nomad-enterprise_${NOMAD_VERSION%2bent}_linux_amd64.zip\
  && curl -L -o nomad_${NOMAD_VERSION%2bent}_SHA256SUMS      https://cdn.aws.robloxlabs.com/nomad/${NOMAD_VERSION%2bent}/nomad-enterprise_${NOMAD_VERSION%2bent}_linux_amd64.zip \
  && curl -L -o nomad_${NOMAD_VERSION%2bent}_SHA256SUMS.sig  https://cdn.aws.robloxlabs.com/nomad/${NOMAD_VERSION%2bent}/nomad-enterprise_${NOMAD_VERSION%2bent}_linux_amd64.zip \
  && export GNUPGHOME="$(mktemp -d)" \
